...
def create_todo(todo: schemas.TodoCreate, db: Session = Depends(deps.get_db)):
...
def update_todo(todo_id: int, todo: schemas.TodoUpdate, db: Session = Depends(deps.get_db)):
